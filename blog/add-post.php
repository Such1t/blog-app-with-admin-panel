...
$avatar_query = "SELECT avatar_url FROM users WHERE id = ?";
$avatar_stmt = $pdo->prepare($avatar_query);
$avatar_stmt->execute([$user_id]);
$user_avatar = $avatar_stmt->fetch(PDO::FETCH_ASSOC);

// If the user has an avatar, use it; otherwise, set a default avatar URL
$avatarUrl = !empty($user_avatar['avatar_url']) ? $user_avatar['avatar_url'] : 'default-avatar.png';
...
<section class="form__section">
    <div class="container form__section-container">
        ...
        <?php endif ?>
        <form id="post-form" action="<?= ROOT_URL ?>add-post-logic.php" enctype="multipart/form-data" method="POST">
...
<script>
    // Customize the image resize module to restrict resizing
    const ImageResizeWithLimit = Quill.import('modules/imageResize');
    class CustomImageResize extends ImageResizeWithLimit {
        onHandleUpdate() {
            const image = this.img;
            const width = parseInt(image.style.width.replace('px', ''));
            const height = parseInt(image.style.height.replace('px', ''));

            // Define your max and min width and height based on container limits
    ...
    function saveFormData() {
        var formData = {
            ...
            is_featured: document.getElementById('is_featured') ? document.getElementById('is_featured').checked : false,
            restricted_to_followers: document.getElementById('restricted_to_followers').checked
        };
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "<?= ROOT_URL ?>save-session.php", true);
