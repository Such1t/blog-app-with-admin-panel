...
// Ensure the user is logged in and that the form was submitted
if (!isset($_SESSION['user-id']) || (!isset($_POST['action']))) {
...
$content = $_POST['content']; // Raw HTML content
$category_id = filter_var($_POST['category'], FILTER_SANITIZE_NUMBER_INT);
$is_featured = isset($_POST['is_featured']) ? 1 : 0;
$restricted_to_followers = isset($_POST['restricted_to_followers']) ? 1 : 0;
$thumbnail = $_FILES['thumbnail'];

// Determine the status based on the action ('update_draft' or 'post_draft')
$status = ($_POST['action'] === 'update_draft') ? 'draft' : 'published';

// Validate input values
if (!$title || !$category_id || !$content) {
    $_SESSION['edit-draft-error'] = "Please fill in all required fields.";
    header('Location: ' . ROOT_URL . 'admin/edit-draft.php?id=' . $post_id);
    exit();
}
...
$query = "UPDATE blog_posts SET title = :title, content = :content, image_url = :image_url, category_id = :category_id, is_featured = :is_featured, restricted_to_followers = :restricted_to_followers, status = :status WHERE id = :id AND user_id = :user_id";
