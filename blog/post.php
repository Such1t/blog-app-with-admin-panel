            ...
            <?php if ($post['restricted_to_followers'] == 0 || $is_following || $isAdmin || $userId === $post['user_id']) : ?>
                <div class="icon-container">
                    ...
                    <div class="comment-count">
                        <i class="uil uil-comments"></i> <span id="comment-count"><?= count($comments) ?></span>
                    </div>
    ...
    <aside class="related-posts">
        ...
        <div class="related-posts__container">
            ...
            if (count($related_posts) > 0) {
                ...
                echo "<p>No other posts by this author.</p>";
            }
            ?>
        </div>
    </aside>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function handleFollowClick(e) {
            ...
            fetch('<?= ROOT_URL ?>follow.php', {
                ...
                .then(response => response.json())
                .then(data => {
                    console.log('Follow Response:', data);  // Log the response
                    if (data.status === 'success') {
        ...
        function submitCommentForm(form) {
            ...
            fetch('<?= ROOT_URL ?>add-comment.php', {
                ...
                .then(response => response.json())
                .then(data => {
                    console.log('Comment Response:', data);  // Log the response
                    if (data.status === 'success') {
                        ...
                        if (parentId) {
                            ...
                            parentComment.innerHTML = commentHtml + parentComment.innerHTML;
                        } else {
                            document.getElementById('comments-section').insertAdjacentHTML('afterbegin', commentHtml);
                        }

                        form.reset();
                        updateCommentCount(data.total_comments); // Use the total_comments returned from the server to ensure accuracy
                    } else {
                        showModal2(data.message);
                    }
                })
        ...
        function submitReplyForm(form) {
            ...
            fetch('<?= ROOT_URL ?>add-comment.php', {
                ...
                .then(response => response.json())
                .then(data => {
                    console.log('Reply Response:', data);  // Log the response
                    if (data.status === 'success') {
                        const commentHtml = data.comment_html;
                        const parentId = form.querySelector('input[name="parent_id"]').value;
                        const parentComment = document.querySelector(`.comment[data-comment-id="${parentId}"] .comment-reply-container`);
                        parentComment.innerHTML = commentHtml + parentComment.innerHTML;
                        form.closest('.reply-form-container').style.display = 'block';
                        form.closest('.comment-reply-container').style.display = 'block';
                        form.reset(); // Clear the reply textarea after submission
                        form.remove();
                        updateCommentCount(data.total_comments); // Use the total_comments returned from the server to ensure accuracy
                    } else {
                        showModal2(data.message);
